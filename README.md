...
## 📋 Release and change-failure evidence

Production promotion evidence is generated by the manual
[release evidence workflow](.github/workflows/release-evidence.yml) and follows
[`docs/RELEASE-EVIDENCE.md`](docs/RELEASE-EVIDENCE.md). It records the source
SHA, four immutable image digests, verified GitHub Artifact Attestations,
deployment/Flux revisions, aggregate synthetic result/timing, rollback
detection/recovery timing, and validated DORA measurements. Mutable tags,
secrets, user data, and inconsistent timing are rejected.

A failed post-deploy journey halts the workflow and leaves a pending reviewed
Git/Flux rollback record; it is never silently counted as a success. The
manual [negative rollout workflow](.github/workflows/negative-rollout.yml)
proves this path on an exact disposable k3d cluster by injecting a missing
image, requiring rollout failure, and timing disposable recovery. Production
rollback is never performed by CI or by `kubectl rollout undo`.
